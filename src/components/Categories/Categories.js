  ...
  useEffect(() => {
    const fetchCategoryBlogs = async () => {
      try {
        ...
        setLoading(true);
        const response = await ajaxCall(
          `/posts-category/?site_domain=breatheoffline.com&category_slug=${selectedCategory.slug}`,
          { method: "GET" }
        );
                  ...
                  >
                    <div className="relative h-48">
                      ...
                      <div className="absolute top-4 left-4">
                        <span
                          className="px-3 py-1 rounded-full bg-white/90 text-emerald-600 text-sm font-medium"
